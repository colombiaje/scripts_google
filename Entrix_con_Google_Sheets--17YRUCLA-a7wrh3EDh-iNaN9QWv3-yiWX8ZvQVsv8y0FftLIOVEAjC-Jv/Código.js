...
function getSheet() {
  return SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Entrix");
}
...
function doGet(e) {
  ...
  // return ContentService.createTextOutput("Bienvenido a Entrix API"); // Eliminada o comentada
}
...
function doPost(e) {
  ...
  try {
    if (e.postData && e.postData.type === "application/json") {
      Logger.log("Detected JSON POST");
      params = JSON.parse(e.postData.contents);
    } else if (e.parameter) {
      Logger.log("Detected URL-encoded POST");
      params = e.parameter;
    } else {
      Logger.log("No recognizable POST data format.");
      return ContentService.createTextOutput("Error: No se recibieron parámetros POST")
                           .setMimeType(ContentService.MimeType.TEXT);
    }
  } catch (err) {
    Logger.log("JSON parse error: " + err);
    return ContentService.createTextOutput("Error procesando JSON: " + err)
                         .setMimeType(ContentService.MimeType.TEXT);
  }
  ...
  if (action === 'addPrompt') {
    return addPrompt(params);
  } else if (action === 'updatePrompt') {
    return updatePromptInSheet(params);
  } else if (action === 'deletePrompt') {
    return deletePrompt(params);
  }
